...
def run(graph, url, request_id, app, function):
    data = {'graph': graph, 'request_id': request_id, 'app': app, 'function': function, 'in_edge': True, 'out_edge': False, "trigger_all": True}
...
if __name__ == '__main__':
    ...
    start = time.time()
    result = run(graph, 'http://localhost:20001/run_graph', request_id, 'graph', 'ccpull')
    end = time.time()
    print(result)
