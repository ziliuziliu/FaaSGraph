...
def run(graph, url, request_id, app, function, round_limit):
    data = {'graph': graph, 'request_id': request_id, 'app': app, 'function': function, 'round_limit': round_limit, 'no_change': True, 'in_edge': True, 'out_edge': False, "vertex_array_cnt": 2}
...
if __name__ == '__main__':
    ...
    start = time.time()
    result = run(graph, 'http://localhost:20001/run_graph', request_id, 'graph', 'pr', 20)
    end = time.time()
    print(result)
