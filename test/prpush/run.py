...
def run(graph, url, request_id, app, function, round_limit):
    data = {'graph': graph, 'request_id': request_id, 'app': app, 'function': function, 'round_limit': round_limit, 'no_change': True, 'in_edge': False, 'out_edge': True, 'vertex_array_cnt': 2, 'aggregate_op': 'FLOAT_ADD'}
...
#         with open('{}/{}/vertex_val.bin'.format(config_manager.config['GRAPH'][graph]['DATA_PATH'], startNode), 'rb') as f:
#             for v in range(startNode, startNode + size):
#                 results[v] = byte2float(f.read(4))
#     with open('serverless_pagerank_result.txt', mode='w') as f:
...
if __name__ == '__main__':
    ...
    start = time.time()
    result = run(graph, 'http://localhost:20001/run_graph', request_id, 'graph', 'prpush', 20)
    end = time.time()
    print(result)
    ...
    print('maxc: {}'.format(result['maxc']))
    print('minc: {}'.format(result['minc']))
